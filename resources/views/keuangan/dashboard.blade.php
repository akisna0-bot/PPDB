...
<div class="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50">
    <div class="max-w-7xl mx-auto px-4 py-8">
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-xl font-bold text-blue-600">Rp {{ number_format($stats['total_revenue'], 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-purple-600">{{ $stats['today_payments'] }}</p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div class="flex items-center">
                            <div class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center mr-3">
                                <span class="text-xs font-bold">{{ substr($payment->applicant->user->name ?? 'N/A', 0, 2) }}</span>
                            </div>
                            <div>
                                <p class="font-medium">{{ $payment->applicant->user->name ?? 'N/A' }}</p>
                                <p class="text-sm text-gray-600">{{ $payment->applicant->major->name ?? 'N/A' }}</p>
                            </div>
                        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <a href="{{ route('keuangan.daftar-pembayaran') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
            ...
            <a href="{{ route('keuangan.rekap') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
                <div class="text-center">
                    <div class="text-4xl mb-4">📊</div>
            ...
            <a href="{{ route('keuangan.master-data') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
                <div class="text-center">
                    <div class="text-4xl mb-4">📄</div>
            ...
            <a href="{{ route('keuangan.log-aktivitas') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
                <div class="text-center">
                    <div class="text-4xl mb-4">📝</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
            <a href="{{ route('keuangan.export-excel') }}" class="bg-green-600 text-white rounded-xl shadow-lg p-6 hover:shadow-xl transition text-center">
                <div class="text-4xl mb-4">📊</div>
                <h3 class="font-semibold text-lg mb-2">Export Excel</h3>
                <p class="text-green-100 text-sm">Download laporan dalam format Excel</p>
            </a>
            
            <a href="{{ route('keuangan.export-pdf') }}" class="bg-red-600 text-white rounded-xl shadow-lg p-6 hover:shadow-xl transition text-center">
                <div class="text-4xl mb-4">📄</div>
...
const paymentChart = new Chart(ctx, {
    type: 'line',
    data: {
        labels: {!! json_encode(array_column($paymentTrend, 'date')) !!},
        datasets: [{
            label: 'Pembayaran per Hari',
            data: {!! json_encode(array_column($paymentTrend, 'count')) !!},
