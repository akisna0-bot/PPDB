...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
            ...
            @else
                ...
                <div class="flex items-center justify-between mb-6">
                    <div class="flex items-center {{ in_array($applicant->status, ['SUBMIT', 'VERIFIED', 'PAYMENT_PENDING', 'PAYMENT_VERIFIED', 'FINAL_REVIEW']) || $applicant->final_status ? 'text-green-600' : 'text-gray-400' }}">
                        <div class="w-8 h-8 rounded-full {{ in_array($applicant->status, ['SUBMIT', 'VERIFIED', 'PAYMENT_PENDING', 'PAYMENT_VERIFIED', 'FINAL_REVIEW']) || $applicant->final_status ? 'bg-green-600' : 'bg-gray-300' }} flex items-center justify-center text-white text-sm font-bold">1</div>
                        <span class="ml-2 text-sm font-medium">Pendaftaran</span>
                    </div>
                    
                    <div class="flex-1 h-1 mx-4 {{ in_array($applicant->status, ['VERIFIED', 'PAYMENT_PENDING', 'PAYMENT_VERIFIED', 'FINAL_REVIEW']) || $applicant->final_status ? 'bg-green-600' : 'bg-gray-300' }}"></div>
                    
                    <div class="flex items-center {{ in_array($applicant->status, ['VERIFIED', 'PAYMENT_PENDING', 'PAYMENT_VERIFIED', 'FINAL_REVIEW']) || $applicant->final_status ? 'text-green-600' : 'text-gray-400' }}">
                        <div class="w-8 h-8 rounded-full {{ in_array($applicant->status, ['VERIFIED', 'PAYMENT_PENDING', 'PAYMENT_VERIFIED', 'FINAL_REVIEW']) || $applicant->final_status ? 'bg-green-600' : 'bg-gray-300' }} flex items-center justify-center text-white text-sm font-bold">2</div>
                        <span class="ml-2 text-sm font-medium">Verifikasi</span>
                    </div>
                    
                    <div class="flex-1 h-1 mx-4 {{ $applicant->status == 'PAYMENT_VERIFIED' || $applicant->final_status ? 'bg-green-600' : 'bg-gray-300' }}"></div>
                    
                    <div class="flex items-center {{ $applicant->status == 'PAYMENT_VERIFIED' || $applicant->final_status ? 'text-green-600' : 'text-gray-400' }}">
                        <div class="w-8 h-8 rounded-full {{ $applicant->status == 'PAYMENT_VERIFIED' || $applicant->final_status ? 'bg-green-600' : 'bg-gray-300' }} flex items-center justify-center text-white text-sm font-bold">3</div>
                        <span class="ml-2 text-sm font-medium">Pembayaran</span>
                    </div>
                    
                    <div class="flex-1 h-1 mx-4 {{ $applicant->final_status ? 'bg-green-600' : 'bg-gray-300' }}"></div>
                    
                    <div class="flex items-center {{ $applicant->final_status ? 'text-green-600' : 'text-gray-400' }}">
                        <div class="w-8 h-8 rounded-full {{ $applicant->final_status ? 'bg-green-600' : 'bg-gray-300' }} flex items-center justify-center text-white text-sm font-bold">4</div>
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    @if($applicant->final_status == 'ACCEPTED')
                        <div class="text-center">
                            ...
                            <p class="text-gray-600">{{ $applicant->final_notes ?? 'Selamat bergabung di SMK Bakti Nusantara 666!' }}</p>
                        </div>
                    @elseif($applicant->final_status == 'REJECTED')
                        <div class="text-center">
                            <div class="text-6xl mb-4">😔</div>
                            <h3 class="text-2xl font-bold text-red-600 mb-2">MOHON MAAF</h3>
                            <p class="text-gray-600">{{ $applicant->final_notes ?? 'Anda belum berhasil pada seleksi kali ini.' }}</p>
                        </div>
                    ...
                    @elseif($applicant->status == 'PAYMENT_PENDING')
                        <div class="text-center">
                            <div class="text-6xl mb-4">💰</div>
                            <h3 class="text-xl font-semibold text-green-600 mb-2">Berkas Diverifikasi - Silakan Bayar</h3>
                            <p class="text-gray-600 mb-4">Selamat! Berkas Anda telah diverifikasi oleh {{ $applicant->verifier->name ?? 'Tim Verifikator' }} pada {{ $applicant->verified_at ? $applicant->verified_at->format('d M Y H:i') : '' }}.</p>
                            @if($applicant->catatan_verifikasi)
                                <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                                    <p class="text-sm text-blue-800"><strong>Catatan:</strong> {{ $applicant->catatan_verifikasi }}</p>
                                </div>
                            @endif
                            <p class="text-gray-600 mb-4">Silakan lakukan pembayaran untuk melanjutkan proses pendaftaran.</p>
                            <a href="{{ route('payment.index') }}" class="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 transition">
                                💳 Lakukan Pembayaran
                            </a>
                        </div>
                    ...
                    @elseif($applicant->status == 'REJECTED')
                        <div class="text-center">
                            ...
                            <p class="text-gray-600 mb-3">Berkas Anda ditolak oleh {{ $applicant->verifier->name ?? 'Tim Verifikator' }} pada {{ $applicant->verified_at ? $applicant->verified_at->format('d M Y H:i') : '' }}.</p>
                            @if($applicant->catatan_verifikasi)
                                <div class="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                                    <p class="text-sm text-red-800"><strong>Alasan:</strong> {{ $applicant->catatan_verifikasi }}</p>
                                </div>
                            @endif
                            <p class="text-gray-600 mb-4">Silakan perbaiki berkas sesuai catatan dan hubungi panitia untuk verifikasi ulang.</p>
                            <a href="{{ route('dokumen.index') }}" class="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700 transition">
                                📄 Perbaiki Berkas
                            </a>
                        </div>
                    @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <a href="{{ route('pendaftaran.create') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
                <div class="text-center">
            ...
            <a href="{{ route('status.index') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
                <div class="text-center">
                    <div class="text-4xl mb-4">📊</div>
            ...
            <a href="{{ route('dokumen.index') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
                <div class="text-center">
                    <div class="text-4xl mb-4">📄</div>
                    <h3 class="font-semibold text-lg mb-2">Upload Dokumen</h3>
                    <p class="text-gray-600 text-sm">Upload berkas persyaratan</p>
                    @if($applicant && $applicant->files->count() > 0)
                        <div class="mt-2">
            ...
            @if(in_array($applicant->status, ['VERIFIED', 'PAYMENT_PENDING', 'PAYMENT_VERIFIED']))
            <a href="{{ route('payment.index') }}" class="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition">
                <div class="text-center">
                    <div class="text-4xl mb-4">💳</div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                ...
                <div class="space-y-2">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">{{ $applicant->no_pendaftaran ?? 'Belum ada' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Nama:</span>
                        <span class="font-medium">{{ $applicant->nama_lengkap ?? auth()->user()->name }}</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">
                            @if($applicant->files->count() > 0)
                                <span class="text-green-600">{{ $applicant->files->count() }} file</span>
                            @else
                    ...
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                @if($applicant && $applicant->final_status)
                                    {{ $applicant->decided_at ? \Carbon\Carbon::parse($applicant->decided_at)->format('d M Y') : 'Sudah diumumkan' }}
                                @else
                                    5 April 2025
                                @endif
                            </p>
...
@if($applicant && auth()->user()->role === 'user')
<script>
